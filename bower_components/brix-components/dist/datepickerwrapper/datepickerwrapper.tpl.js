...
<div class="datepickerwrapper <%= mode === 'multiple' ? 'multiple' : 'single' %>">
    <!--  -->
    <% if (mode === 'signal') { %>
    <div bx-name="components/datepicker" data-type="<%= type %>" data-date="<%= dates[0] %>" data-range="<%= _ranges %>" data-unlimit="<%= unlimits[0] %>" class="picker"></div>
    <% } %>
    <!--  -->
    <% if (mode === 'multiple') { %>
    <% if( shortcuts ) { %>
    ...
    <div class="datepickerwrapper-pickers">
        <% for (var i = 0; i < dates.length; i++ ) { %>
            <div bx-name="components/datepicker" data-date="<%= dates[i] %>" data-range="<%= _ranges %>" data-unlimit="<%= unlimits[i] %>" data-type="<%= type %>" class="picker"></div>
        <% } %>
    </div>
